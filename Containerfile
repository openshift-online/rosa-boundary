...
# ║    fzf-builder      → fzf binary + shell integration from GitHub Releases  ║
...
# ══════════════════════════════════════════════════════════════════════════════
# Stage 5: fzf-builder
# Downloads fzf from the junegunn/fzf GitHub Releases with checksum
# verification, plus shell integration scripts from the tagged source.
# Output: /opt/fzf/bin/fzf and /opt/fzf/shell/*.bash
# ══════════════════════════════════════════════════════════════════════════════
FROM tools-base AS fzf-builder

# Pin fzf version — Renovate updates this ARG via PR
ARG FZF_VERSION="0.73.1"

ARG OUTPUT_DIR="/opt"
ENV FZF_URL_SLUG="junegunn/fzf"
ENV FZF_URL="https://api.github.com/repos/${FZF_URL_SLUG}/releases/tags/v${FZF_VERSION}"
ENV FZF_CHECKSUM_FILE="fzf_${FZF_VERSION}_checksums.txt"
ENV FZF_CHECKSUM_ALGORITHM="sha256"
ENV FZF_PLATFORM_PREFIX="linux_"

RUN mkdir --parents /fzf-dl /opt/fzf/bin /opt/fzf/shell
WORKDIR /fzf-dl

# Download fzf binary with checksum verification via github_dl
RUN --mount=type=secret,id=GITHUB_TOKEN \
    --mount=type=secret,id=read-only-github-pat/token \
    github_dl download \
        --url "${FZF_URL}" \
        --checksum_file "${FZF_CHECKSUM_FILE}" \
        --checksum_algorithm "${FZF_CHECKSUM_ALGORITHM}" \
        --platform "${FZF_PLATFORM_PREFIX}$(platform_convert "@@PLATFORM@@" --amd64 --arm64)"

# Extract binary
RUN tar --extract --gzip --file ./*.tar.gz --directory /opt/fzf/bin \
    && chmod +x /opt/fzf/bin/fzf

# Download shell integration scripts from the tagged source
# These provide Ctrl-R (history search) and Ctrl-T (file finder) keybindings
RUN --mount=type=secret,id=GITHUB_TOKEN \
    --mount=type=secret,id=read-only-github-pat/token \
    TOKEN="" \
    && if [ -f /run/secrets/read-only-github-pat/token ]; then \
    ...
    && for script in key-bindings.bash completion.bash; do \
        curl --silent --location --fail \
            ${AUTH_HEADER:+--header "${AUTH_HEADER}"} \
            "https://raw.githubusercontent.com/junegunn/fzf/v${FZF_VERSION}/shell/${script}" \
            --output "/opt/fzf/shell/${script}"; \
    done
...
# Required for SSM/ECS Exec sessions. Installed from AWS S3 RPM.
# Architecture mapping: x86_64 → "64bit", aarch64 → "arm64"
COPY --from=tools-base /usr/local/bin/platform_convert /tmp/platform_convert
RUN rpm --install \
        "$(/tmp/platform_convert \
            "https://s3.amazonaws.com/session-manager-downloads/plugin/latest/linux_@@PLATFORM@@/session-manager-plugin.rpm" \
            --arm64 --custom-amd64 64bit)" \
    && rm --force /tmp/platform_convert
...
# ── fzf ─────────────────────────────────────────────────────────────────────
# Interactive fuzzy finder. Shell scripts provide Ctrl-R and Ctrl-T bindings.
COPY --from=fzf-builder /opt/fzf/bin/fzf    /usr/local/bin/fzf
COPY --from=fzf-builder /opt/fzf/shell      /usr/share/fzf/shell
...
# Contains .bashrc, .bashrc.d/*, .inputrc, .claude/, .local/bin/sre-login.
COPY skel/sre/ /etc/skel-sre/
RUN chmod +x /etc/skel-sre/.local/bin/sre-login
