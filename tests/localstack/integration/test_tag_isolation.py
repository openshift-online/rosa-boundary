...
def test_cross_user_isolation_via_policy_simulation(iam_client):
    """Test that the ABAC policy enforces cross-user isolation.

    Uses iam:SimulatePrincipalPolicy to verify:
    - Alice (session tag username=alice) is ALLOWED to exec into her own task (tagged username=alice)
    - Alice is DENIED exec into Bob's task (tagged username=bob)
    - Missing session tag results in deny (fail-closed)

    This tests the runtime ABAC behavior, not just the policy document structure.
    ...
    """
    role_name = f'test-isolation-sim-{int(datetime.now().timestamp())}'
    cluster_arn = 'arn:aws:ecs:us-east-2:123456789012:cluster/test-cluster'
    ...
    alice_task_arn = 'arn:aws:ecs:us-east-2:123456789012:task/test-cluster/alice-task-001'
    bob_task_arn = 'arn:aws:ecs:us-east-2:123456789012:task/test-cluster/bob-task-002'

    # Simulate alice's session (username session tag = alice)
    alice_context = [
    ...
    alice_on_own_task = iam_client.simulate_principal_policy(
        PolicySourceArn=role_arn,
        ActionNames=['ecs:ExecuteCommand'],
        ResourceArns=[alice_task_arn],
        ContextEntries=alice_context
    )
    assert alice_on_own_task['EvaluationResults'][0]['EvalDecision'] == 'allowed', (
        "Alice must be allowed to exec into her own task (username tag matches session tag)"
    )

    # Simulate alice trying to exec into bob's task (ResourceTag/username = bob)
    alice_on_bob_task_context = [
        {'ContextKeyName': 'aws:PrincipalTag/username', 'ContextKeyValues': ['alice'], 'ContextKeyType': 'string'},
        {'ContextKeyName': 'ecs:ResourceTag/username', 'ContextKeyValues': ['bob'], 'ContextKeyType': 'string'},
    ]
    alice_on_bob_task = iam_client.simulate_principal_policy(
        PolicySourceArn=role_arn,
        ActionNames=['ecs:ExecuteCommand'],
        ResourceArns=[bob_task_arn],
        ContextEntries=alice_on_bob_task_context
    )
    assert alice_on_bob_task['EvaluationResults'][0]['EvalDecision'] != 'allowed', (
    ...
    no_tag_result = iam_client.simulate_principal_policy(
        PolicySourceArn=role_arn,
        ActionNames=['ecs:ExecuteCommand'],
        ResourceArns=[alice_task_arn],
        ContextEntries=no_session_tag_context
    )
    assert no_tag_result['EvaluationResults'][0]['EvalDecision'] != 'allowed', (
        "Missing session tag must result in deny (fail-closed: empty PrincipalTag != task username)"
    )
