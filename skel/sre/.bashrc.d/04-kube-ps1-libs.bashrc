...
#
# Vendored from jonmosco/kube-ps1 via openshift/ocm-container.
...
_kube_ps1_color_bg() {
    ...
    *) KUBE_PS1_BG_CODE=$'\033[0m' ;;
    ...
    if [[ "${KUBE_PS1_BG_CODE}" == "default" ]]; then
        KUBE_PS1_BG_CODE="${_KUBE_PS1_DEFAULT_BG}"
        return
    elif [[ "${KUBE_PS1_SHELL}" == "zsh" ]]; then
...
kubeon() {
    if [[ "${1}" == '-h' || "${1}" == '--help' ]]; then
        ...
        rm --force -- "${KUBE_PS1_DISABLE_PATH}"
    elif [[ "$#" -ne 0 ]]; then
...
kubeoff() {
    if [[ "${1}" == '-h' || "${1}" == '--help' ]]; then
        ...
        mkdir --parents -- "$(dirname "${KUBE_PS1_DISABLE_PATH}")"
